...
**含义**：`pages/functions/_middleware.js` + `pages/wrangler.toml` 的 `[[services]] BACKEND` + `service="cloudflare_temp_email"`（与线上 worker 名 one-mail 不匹配）是**从未生效的空转配置**。「前后端分离」的正确落地 = 面向首次部署：把仓库改成「前端一上线就直连 worker」，而不是迁移一个不存在的线上拓扑。

## 现状耦合点（源码 + 线上实测）

1. **Pages 代理拓扑空转**：`pages/wrangler.toml:7` `service="cloudflare_temp_email"` 在 CF 上查无此名（线上 worker 是 `one-mail`）。`_middleware.js` + service binding 从未被部署使用，维护它就是纯负担。
...
- **A. whica `frontend_pagefunction_deploy.yaml` 删除还是标注废弃**：从仓库角度看，page function 方案已废弃，建议**删除 workflow**避免误导未来部署；若用户倾向于保留上游兼容，则仅加注释标注废弃+不改。
...
- `git grep` 确认仓库内 `API_PATHS` 只剩 `worker/src/worker.ts` 一处；`service="cloudflare_temp_email"` 无残留。
