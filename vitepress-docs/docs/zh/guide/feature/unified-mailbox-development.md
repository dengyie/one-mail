...
| 实时收件 | 已具备（IMAP IDLE） | 支持的账号由常驻 IDLE 监听线程秒级推送到位；POP3 / graph 等账号 60s 兜底轮询 |
| 立即同步/连接测试 | 无 | 用户只能等待下一轮兜底轮询（多数账号已由 IDLE 实时同步） |
...
- 支持 IMAP IDLE 的账号已由常驻监听线程实时收信；POP3 / graph 等无 IDLE 的账号仍是 60s 兜底轮询，验证码场景对这类账号仍有秒级到分钟级延迟；
