...
- pages 前端 `secrets`
   ...
   | `FRONTEND_NAME`    | 你在 Cloudflare Pages 创建的项目名称，可通过 [用户界面](https://temp-mail-docs.pages.dev/zh/guide/ui/pages.html) 或者 [命令行](https://temp-mail-docs.pages.dev/zh/guide/cli/pages.html) 创建 |
...
> [!warning] 安全提示
> `CLOUDFLARE_API_TOKEN`、`CLOUDFLARE_ACCOUNT_ID` 和 `BACKEND_TOML` 必须放在 Repository secrets 中。部署工作流不会接受在命令行或 workflow 输入中粘贴明文 Token；这样可以避免凭据出现在 workflow 运行参数和日志关联数据中。

### 部署

- 打开仓库的 `Actions` 页面
- 确认上面的后端 Secrets 已配置，再找到 `Deploy Backend` 点击 `Run workflow` 选择分支手动部署
...
- 如果后端 Secrets 缺失，`Deploy Backend` 会在部署前直接失败并提示缺少的变量，不会执行半配置部署

### 自动更新

如果你既想让 Pages 通过 Page Functions 转发后端请求，请使用 `Deploy Frontend with page function` workflow，而不是 `Deploy Frontend`。
