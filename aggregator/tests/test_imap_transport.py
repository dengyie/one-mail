...
def test_create_imap_client_uses_socks_transport_for_overseas_host():
    calls = []
    ...
    client = create_imap_client(
        _account("IMAP.GMAIL.COM"),
        timeout=30,
        direct_client_cls=_Direct,
        proxied_client_cls=_Proxy,
    )
    assert isinstance(client, _Proxy)
    assert calls == [("proxy", ("IMAP.GMAIL.COM",), {"port": 993, "ssl": True, "timeout": 30})]
