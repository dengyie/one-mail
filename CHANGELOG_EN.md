...
- fix: |Aggregator| IMAP sync OOM on huge-attachment mailboxes: a QQ INBOX contains single 66MB/65MB messages; batching by count (`BATCH_SIZE=200`) pulled ~200MB+ of RFC822 per window into memory and pxed's 4GB VPS repeatedly OOM-killed python (`rc=137`), stalling sync at `last_uid=4024`. `fetch_new_messages` now probes `RFC822.SIZE` first and caps each window at `BATCH_BYTES=64MiB` (still including an individual oversize message so the mailbox can never deadlock); real window 4025 was truncated from 200 msgs → 149 msgs ≈84MB and resumed converging (review fix round 3, commit `269c3fd`)
