...
- fix: |Aggregator| Regression fix when an entire POP3 window fails to normalize (review Important-1): `sync_pop3` used to return 0 silently when every pending message failed, so `_fallback_to_pop3` treated it as a successful POP3 sync and permanently pinned the account to POP3. It now raises `RuntimeError` (nothing uploaded, nothing marked seen), so an `auto` account retries IMAP next round and an explicit pop3 account retries the same unseen UIDLs. Also added upload-failure watermark boundary tests: an IMAP upload error does not advance the watermark (next round re-pulls the same window); a POP3 upload error marks no UIDL seen
