...
- fix: |Aggregator| POP3 全窗口归一化失败的回归修复（review Important-1）：`sync_pop3` 原先整批解析失败时静默返回 0，`_fallback_to_pop3` 会把「POP3 同步成功」误判并永久钉住账号到 POP3。现改为抛出 `RuntimeError`（什么都不上传、什么都不标记 seen），auto 账号下轮重试 IMAP，显式 pop3 账号由 `run_once` 捕获后下轮重试同批未 seen 的 UIDL。另补 upload 失败水印边界测试：IMAP upload 抛错 → 水印不推进（下轮续传）；POP3 upload 抛错 → 不标记任何 UIDL seen
